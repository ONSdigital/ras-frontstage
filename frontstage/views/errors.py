...
def not_logged_in_error_page():
    return render_template('errors/not-signed-in.html', data={"error": {"type": "failed"}}), 403


@error_bp.route('/404', methods=['GET', 'POST'])
def not_found_error_page():
    return render_template('errors/404-error.html', data={"error": {"type": "failed"}}), 404


@error_bp.route('/500', methods=['GET', 'POST'])
def server_error_page():
    return render_template('errors/500-error.html', data={"error": {"type": "failed"}}), 500
