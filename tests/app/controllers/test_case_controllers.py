...
class TestCaseControllers(unittest.TestCase):
    ...
    def test_get_eq_url_case_group_status_not_complete(self, get_case_by_id, create_eq_payload, *_):
        ...
        with app.app_context():
            eq_url = case_controller.get_eq_url(case['id'], respondent_party['id'], business_party['id'], survey_eq['shortName'])
    ...
    def test_post_case_event_success(self, _):
        ...
        with responses.RequestsMock() as rsps:
            ...
            with app.app_context():
                try:
                    case_controller.post_case_event(case['id'], respondent_party['id'], message['category'], message['description'])
                except ApiError:
    ...
    def test_get_case_data_success(self, get_collection_exercise, get_collection_instrument, get_survey_by_short_name,
        ...
        with app.app_context():
            case_data = case_controller.get_case_data(case['id'], respondent_party['id'], business_party['id'], survey['shortName'])
    ...
    @patch('frontstage.controllers.collection_instrument_controller.get_collection_instrument')
    def test_link_enrolment_to_cases_success(self, get_collection_instrument):
        get_collection_instrument.return_value = collection_instrument_seft
        ...
        linked_enrolment_to_case = case_controller.link_enrolment_to_cases(enrolment, case_list)

        self.assertTrue(linked_enrolment_to_case['case_id'] is not None)
        self.assertTrue(linked_enrolment_to_case['collection_instrument'] is not None)
        self.assertTrue(linked_enrolment_to_case['status'] is not None)

    def test_link_enrolment_to_case_no_case_to_be_linked(self):
        ...
        linked_enrolment_to_case = case_controller.link_enrolment_to_cases(enrolment, [case])

        self.assertTrue(linked_enrolment_to_case is None)

    @patch('frontstage.controllers.case_controller.link_enrolment_to_cases')
    @patch('frontstage.controllers.case_controller.get_cases_for_list_type_by_party_id')
    def test_get_enrolments_with_cases(self, get_cases, link_enrolment_with_case):
        ...
        link_enrolment_with_case.return_value = enrolments_with_cases

        filtered_enrolments_with_cases = case_controller.get_enrolments_with_cases(enrolments, 'todo')

        self.assertNotIn(None, filtered_enrolments_with_cases)
