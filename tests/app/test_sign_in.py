...
with open('tests/test_data/respondent/suspended_respondent.json') as json_data:
    suspended_respondent_data = json.load(json_data)
...
class TestSignIn(unittest.TestCase):
    ...
    def test_sign_in_success(self, mock_object):
        mock_object.get(get_respondent_by_email, json=respondent_data, status_code=200)
    ...
    def test_sign_in_unknown_response(self, mock_object):
        self.oauth_error['error']['data']['detail'] = 'failed'
        mock_object.get(get_respondent_by_email, json=respondent_data, status_code=200)
        mock_object.post(url_oauth_token, status_code=401, json=self.oauth_error)
    ...
    @requests_mock.mock()
    def test_sign_in_suspended_account(self, mock_object):
        self.oauth_error['error']['data']['detail'] = 'failed'
        mock_object.get(get_respondent_by_email, json=suspended_respondent_data, status_code=200)
