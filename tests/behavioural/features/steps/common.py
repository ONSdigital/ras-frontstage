...
def step_impl_body_empty_error(context):
...
def step_impl_subject_and_body_error(context):
    step_impl_subject_empty_error(context)
    step_impl_body_empty_error(context)


@then('I should receive a body too long error')
def step_impl_body_too_long_error(context):
    context.browser.find_by_link_text('Body field length must not be greater than 10000')


@then('I should receive a subject too long error')
def step_impl_subject_too_long__error(context):
    context.browser.find_by_link_text('Subject field length must not be greater than 100')


@then('I should receive subject and body too long errors')
def step_impl_subject_and_body_too_long_error(context):
    step_impl_subject_too_long(context)
