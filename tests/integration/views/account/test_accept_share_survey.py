...
url_post_pending_shares = f"{app.config['PARTY_URL']}/party-api/v1/share-surveys"
url_get_survey_second = f"{app.config['SURVEY_URL']}/surveys/02b9c366-7397-42f7-942a-76dc5876d86d"
url_get_pending_shares_by_batch = f"{app.config['PARTY_URL']}/party-api/v1/share-survey/{batch_number}"
url_get_shared_to_respondent_party = f"{app.config['PARTY_URL']}/party-api/v1/respondents/email"
url_post_share_survey_respondent = f"{app.config['PARTY_URL']}/party-api/v1/share-survey-respondent"
...
class TestAcceptShareSurvey(unittest.TestCase):
    ...
    def test_post_accept_share_surveys_success_non_existing_account(self, mock_request):
        ...
        response = self.app.post(f'/register/share-surveys/create-account/enter-account-details?batch_no={batch_number}'
                                 f'&email=test@test.com',
                                 data=data,
                                 follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn("Thank you".encode(), response.data)
        self.assertIn("You are now registered to access the business survey/s shared with you.".encode(), response.data)
