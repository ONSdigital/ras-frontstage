...
class TestSurveyList(unittest.TestCase):
    ...
    def test_transfer_survey_options_selection(self, mock_request, get_respondent_party_by_id):
        ...
        self.assertTrue('What will happen'.encode() in response.data)
        self.assertTrue(
            'Select which surveys you want to transfer.'.encode() in response.data)
        self.assertTrue('Enter the email address of the person who will be responding to these surveys.'.
                        encode() in response.data)
        self.assertTrue('We will email them the instructions to access the surveys.'.encode() in response.data)
        self.assertTrue('Once we confirm their access, they will be able to respond to the surveys and share access '
